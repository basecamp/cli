...
BINARY_NAME="$(basename "$PROJECT_ROOT")"
...
for pkg in $PACKAGES; do
    ...
    go test -cpuprofile="$profile_file" \
        -bench=. \
        -benchtime=3s \
        -count=1 \
...
if [[ -n "$PROFILE_FILES" ]]; then
    # shellcheck disable=SC2086 # intentional word-split — PROFILE_FILES is space-delimited list
...
echo ""
echo "Build with PGO:"
echo "    go build -pgo=auto ./cmd/$BINARY_NAME"
echo "    # or"
echo "    make build-pgo"
